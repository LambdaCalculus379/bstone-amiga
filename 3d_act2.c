...
// Unused array from Wolfenstein 3D (used in Catacomb 3-D)
...
int16_t	far starthitpoints[4][NUMHITENEMIES] =
	 //
	 // BABY MODE
	 //
{
...
// Some AOG vs PS differences
#ifdef GAMEVER_RESTORATION_AOG
uint16_t far BossShotShapes[] = {SPR_BOSS1_PROJ1,0,0,0,SPR_BOSS5_PROJ1};
#else
uint16_t far BossShotShapes[] = {SPR_BOSS1_PROJ1,0,0,0,SPR_BOSS5_PROJ1,0,0,SPR_BOSS10_SPIT1};
#endif
uint16_t far BossShapes[] = /*** BLAKE STONE: ALIENS OF GOLD RESTORATION ***/
#ifdef GAMEVER_RESTORATION_AOG
...
uint16_t far MorphShapes[] = {SPR_BOSS1_MORPH1, SPR_BOSS4_MORPH1, SPR_MUTHUM2_MORPH1};
uint16_t far MorphClass[] = {spider_mutantobj,reptilian_warriorobj,mutant_human2obj};
uint16_t far MorphEndShapes[] = {SPR_BOSS1_W1,SPR_BOSS4_W1,SPR_MUTHUM2_W1};
int8_t far MorphSounds[] = {SCANHALTSND,GGUARDHALTSND,DOGBOYHALTSND};
uint16_t bars_connected = 0;
#endif

/*** BLAKE STONE: ALIENS OF GOLD RESTORATION ***/
#ifndef GAMEVER_RESTORATION_AOG
uint16_t SpecialSpawnFlags[] = {FL2_DROP_RKEY,FL2_DROP_YKEY,FL2_DROP_BKEY,
...
/*** BLAKE STONE: ALIENS OF GOLD V1.0 RESTORATION ***/
// HACK - Redefining T_Shade as NULL for state definitions
#ifdef GAMEVER_RESTORATION_AOG_100
#define T_Shade NULL
#else
...
int16_t RandomSphereDir(enemy_t dir);
...
int16_t far grenade_shapes[] = {SPR_GRENADE_FLY3,SPR_GRENADE_FLY3,SPR_GRENADE_FLY2,
...
/*** BLAKE STONE: ALIENS OF GOLD RESTORATION ***/
	...
	case ventdripobj:
		...
		/*** BLAKE STONE: ALIENS OF GOLD V1.0 RESTORATION ***/
#ifdef GAMEVER_RESTORATION_AOG_100
				obj->temp1 = SPR_DECO_ARC_2+(US_RndT()&1);
#else
				int8_t t = (US_RndT()&1);
...
// RandomSphereDir()
//---------------------------------------------------------------------------
int16_t RandomSphereDir(enemy_t enemy)
{
	int16_t dir;
...
/*** BLAKE STONE: ALIENS OF GOLD V1.0 RESTORATION ***/
// Enable the IN_DEVELOPMENT specific checks
				case gurney_waitobj:
#ifdef OBJ_RESERV
#if (IN_DEVELOPMENT) || (defined GAMEVER_RESTORATION_AOG_100)
//#if IN_DEVELOPMENT
...
/*** BLAKE STONE: ALIENS OF GOLD RESTORATION ***/
	...
	if (new->obclass!=blakeobj)
	{
		if ((uint16_t)actorat[new->tilex][new->tiley] == 1)
...
void T_Chase (objtype *ob)
{
	...
	/*** BLAKE STONE: ALIENS OF GOLD RESTORATION ***/
	// Uncomment code for AOG v2.0 and later
...
int16_t CalcAngle(objtype *from_obj, objtype *to_obj);
//---------------------------------------------------------------------------
// SpawnProjectile()
//---------------------------------------------------------------------------
...
/*** BLAKE STONE: ALIENS OF GOLD RESTORATION ***/
#ifndef GAMEVER_RESTORATION_AOG
							proj_wall = (uint8_t)(intptr_t)proj_check;	 
#endif
							proj_check=false;
							ob->tilex = ob->x>>TILESHIFT;
							ob->tiley = ob->y>>TILESHIFT;
...
/*** BLAKE STONE: ALIENS OF GOLD RESTORATION ***/
#ifdef GAMEVER_RESTORATION_AOG
int8_t ff_damageplayer,ff_damage;
#else
int16_t ff_damageplayer,ff_damage;
#endif
objtype *ff_obj;

void ExplodeFill(int8_t tx, int8_t ty);
...
void T_BlowBack(objtype *obj)
{
	...
	if (!(obj->flags & FL_SLIDE_INIT))
	{
		...
		killer = (objtype *)ui16_to_actor(SLIDE_TEMP(obj));
		if (!killer)
		{
		...
		if ((killer = (objtype *)ui16_to_actor(SLIDE_TEMP(obj))) == player)
