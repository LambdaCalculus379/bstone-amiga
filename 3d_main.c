...
//unsigned long __attribute__((used)) __stack=0x8000;
unsigned long __attribute__((used)) __stack=0x10000;
#endif
...
int8_t lastmap_tilex,lastmap_tiley;
#endif
...
void WriteConfig(void)
{
	...
	if (file != -1)
	{
		write(file,Scores,sizeof(HighScore) * MaxScores);//e

		write(file,&SoundMode,sizeof(SoundMode)); // se enum
		write(file,&MusicMode,sizeof(MusicMode));
		write(file,&DigiMode,sizeof(DigiMode));

		write(file,&mouseenabled,sizeof(mouseenabled)); // se BOOLEAN!!
		write(file,&joystickenabled,sizeof(joystickenabled));
		write(file,&joypadenabled,sizeof(joypadenabled));
		write(file,&joystickprogressive,sizeof(joystickprogressive));
		write(file,&joystickport,sizeof(joystickport)); // e

		write(file,&dirscan,sizeof(dirscan)); // e
		write(file,&buttonscan,sizeof(buttonscan));
		write(file,&buttonmouse,sizeof(buttonmouse));
		write(file,&buttonjoy,sizeof(buttonjoy));

		write(file,&viewsize,sizeof(viewsize)); // e
		write(file,&mouseadjustment,sizeof(mouseadjustment));

		write(file,&gamestate.flags,sizeof(gamestate.flags));		// e32
...
boolean LevelInPlaytemp(int8_t levelnum);
...
// DoChecksum()
//--------------------------------------------------------------------------
int32_t DoChecksum(byte far *source,uint16_t size,int32_t checksum)
{
	uint16_t i;
...
// FindChunk()
//--------------------------------------------------------------------------
int32_t FindChunk(int file, char *chunk)
{
	int32_t chunklen;
...
// ReadInfo()
//--------------------------------------------------------------------------
void ReadInfo(boolean decompress,char far *dst, uint16_t size, int file)
{
	uint16_t csize,dsize;
...
// WriteInfo()
//--------------------------------------------------------------------------
uint16_t WriteInfo(boolean compress, char far *src, uint16_t size, int file)
{
	uint16_t csize;
...
//
	...
	for (int i = 0; i < MAPSIZE; ++i)
	{
		...
		{
			intptr_t value = (intptr_t)actorat[i][j];
			if (value < 0)
				actorat[i][j] = &objlist[-value];
			else
				actorat[i][j] = (objtype *)(value);
		}
	}
#endif
...
#ifdef __AMIGA__
	memcpy(new,ptr,offsetof(objtype,next));		// don't copy over links!
...
#ifdef __AMIGA__
	if ((intptr_t)laststatobj < 0)
		laststatobj = NULL;
	else
		laststatobj = &statobjlist[(intptr_t)laststatobj];

	for (int i = 0; i < MAXSTATS; ++i)
	{
		intptr_t value = statobjlist[i].visspot;
		if (value < 0)
			statobjlist[i].visspot = NULL;
		else
			statobjlist[i].visspot = &(&spotvis[0][0])[value];
	}
#endif
...
#ifdef __AMIGA__
	for (int i = 0; i < MAX_CACHE_MSGS; ++i)
		ConHintList.cmInfo[MAX_CACHE_MSGS].mInfo.mSeg = NULL;
#endif
...
//
	...
	{
		int16_t old_wx=WindowX,old_wy=WindowY,old_ww=WindowW,old_wh=WindowH,
			 old_px=px,old_py=py;
...
// SaveLevel()
//--------------------------------------------------------------------------
boolean SaveLevel(int16_t levelnum)
{
	objtype *ob;
	int handle;
	struct ffblk finfo;
	...
	char chunk[5]="LVxx";
	uint16_t gflags = gamestate.flags;
...
#ifdef __AMIGA__
	uint32_t tmp32 = 0xDEADBEEF;
	write(handle,&tmp32,4); // four bytes dummy data
#else
...
#ifdef __AMIGA__
	objtype *actorsave[MAPSIZE][MAPSIZE]; // 4k stack!
	for (int i = 0; i < MAPSIZE; ++i)
	{
		for (int j = 0; j < MAPSIZE; ++j)
		{
			actorsave[i][j] = actorat[i][j];
			if (actorat[i][j] >= objlist)
				actorat[i][j] = (objtype *)(-(intptr_t)(actorat[i][j] - objlist));
			/*else
				actorat[i][j] = (intptr_t)actorat[i][j];*/
		}
	}
#endif
...
//
	...
	{
		ob = &((objtype *)temp)[i];
		ob->state = (statetype *)get_state_index(ob->state);
	}
#endif
...
#ifdef __AMIGA__
	intptr_t tmp = (intptr_t)(laststatobj - statobjlist);
	WriteIt(false, &tmp, sizeof(tmp));
	for (int i = 0; i < MAXSTATS; ++i)
		statobjlist[i].visspot = (intptr_t)statobjlist[i].visspot - (intptr_t)&spotvis[0][0];
#else
...
//
	...
	write(handle,&cksize,4);
	cksize = SWAP32LE(cksize);
	lseek(handle,cksize,SEEK_CUR);//goto cleanup;
   ...
   for (y=0;y<mapheight;y++)
	   for (x=0;x<mapwidth;x++)
      {
      	if (*tile_ptr & 0x80)
         {
            ...
            obj = *actor_ptr;
			/*uint16_t actor_u16 = (uint16_t)(size_t)obj;
...
void ClearNClose()
{
	int8_t x,y,tx=0,ty=0,px=player->x>>TILESHIFT,py=player->y>>TILESHIFT;
	...
	if (tx)
	{
		int8_t doornum=tilemap[tx][ty]&63;

		doorobjlist[doornum].action = dr_closed;		// this door is closed!
		doorposition[doornum]=0;							// draw it closed!
		//(uint16_t)actorat[tx][ty] = doornum | 0x80;	// make it solid!
		actorat[tx][ty] = (objtype *)(doornum | 0x80);	// make it solid!
	}
}
...
//	StopMusic();
   ...
   if (palette)
   	...
   	Movies[movie].palette = (memptr)vgapal;
#endif
...
#ifdef __AMIGA__
	view_xl=(320-viewwidth)/2;
	view_xh=viewwidth;
	view_yl=(200-STATUSLINES-viewheight+TOP_STRIP_HEIGHT)/2;
	view_yh=viewheight;
	// proper alignment for the C2P
	int diff = view_xl % 8;
	if (diff != 0)
	{
		view_xl -= diff;
...
void ShowViewSize (int16_t width)
{
	...
	viewwidth = width*16;
#ifdef __AMIGA__
	viewheight = floor(width*16*HEIGHTRATIO);
#else
...
void NewViewSize (int16_t width)
{
	...
	{
#ifdef __AMIGA__
		if (SetViewSize (width*16,floor(width*16*HEIGHTRATIO)))
#else
...
//#if 0
	if (!error || !(*error))
	{
		uint16_t far *clear = MK_FP(0xb800,23*80*2);
		uint16_t len = 0;
...
#else
//			movedata ((uint16_t)screen,0,0xb800,0,4000);
...
/*** BLAKE STONE: ALIENS OF GOLD V1.0 RESTORATION ***/
#ifdef GAMEVER_RESTORATION_AOG_100
int16_t starting_episode=0,starting_level=1,starting_difficulty=2;
#endif
#if IN_DEVELOPMENT || TECH_SUPPORT_VERSION
...
int main (int argc, char *argv[])
{
	...
	struct stat st;
	if (stat("adlib", &st) == -1 || !S_ISDIR(st.st_mode))
	{
		// time to convert the sounds
		system("convert-sounds");
	}
#endif
