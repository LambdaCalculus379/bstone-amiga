...
#ifndef __AMIGA__
asm	mov	ax,0x4300
asm	int16_t	XMS_INT         				// Check for presence of XMS driver
	if (_AL != 0x80)
		goto error;

asm	mov	ax,0x4310
asm	int16_t	XMS_INT							// Get address of XMS driver
...
asm	mov	[WORD PTR XMSDriver+2],es		// function pointer to XMS driver
	...
	XMSAvail = _AX;
	/*** BLAKE STONE: ALIENS OF gGOLD V1.0 RESTORATION ***/
	// What preceded AJR bugfix (also in Wolfenstein 3D)?
#ifdef GAMEVER_RESTORATION_AOG_100
	if (_BL)
#else
	if (!_AX)
...
PML_XMSCopy(boolean toxms,byte far *addr,word xmspage,word length)
{
	...
	copy.source_offset = toxms? (int32_t)addr : xoffset;
	copy.target_handle = toxms? XMSHandle : 0;
	copy.target_offset = toxms? xoffset : (int32_t)addr;
...
//
void
PML_CopyToXMS(byte far *source,int16_t targetpage,word length)
{
...
//
void
PML_CopyFromXMS(byte far *target,int16_t sourcepage,word length)
{
...
PML_ShutdownXMS(void)
{
	...
	if (XMSPresent)
	{
   	...
   	XMSPresent = false;
		...
		XMS_CALL(XMS_FREE);
		/*** BLAKE STONE: ALIENS OF GOLD V1.0 RESTORATION ***/
		// What preceded bugfix from Wolfenstein 3D,
		// in PML_StartupXMS, just Blake Stone specific?
#ifdef GAMEVER_RESTORATION_AOG_100
		if (_BL)
#else
		if (!_AX)
...
void PML_ReadFromFile(byte far *buf,int32_t offset,word length)
{
#ifdef FLASH_LOAD
...
memptr
PML_GetEMSAddress(int16_t page,PMLockType lock)
{
	int16_t		i,emspage;
...
//
void
PML_PutPageInXMS(int16_t pagenum)
{
	int16_t				usexms;
	PageListStruct	far *page;
...
//
memptr
PML_TransferPageSpace(int16_t orig,int16_t new)
{
	memptr			addr;
...
//
byte far *
PML_GetAPageBuffer(int16_t pagenum,boolean mainonly)
{
	byte			far *addr = nil;
	int16_t				i,n;
	PMBlockAttr		*used;
...
//
memptr
PML_GetPageFromXMS(int16_t pagenum,boolean mainonly)
{
	byte			far *checkaddr;
	memptr			addr = nil;
	PageListStruct	far *page;
...
//
void
PML_LoadPage(int16_t pagenum,boolean mainonly)
{
	byte			far *addr;
	PageListStruct	far *page;
...
#ifdef __AMIGA__
#ifndef __CYGWIN__
	// convert unsigned samples to signed for AHI
	if (pagenum >= PMSoundStart && pagenum < ChunksInFile - 1)
	{
		//printf("%s(%d,%d) fixing samples\n", __FUNCTION__, pagenum, mainonly);
		for (word i = 0; i < page->length; i++)
		{
			addr[i] ^= 128;
		}
	}
...
#endif
	if (pagenum >= PMSpriteStart && pagenum < PMSoundStart)
	{
		//printf("%s(%d,%d) byteswapping sprite\n", __FUNCTION__, pagenum, mainonly);
