...
static void DeleteNode(int16_t p);  /* Deleting node from the tree */
static void InsertNode(int16_t r);  /* Inserting node to the tree */
static void InitTree(void);  /* Initializing tree */
static void Putcode(intptr_t outfile_ptr, int16_t l, uint16_t c,uint16_t PtrTypes);		/* output c bits */
static void EncodeChar(intptr_t outfile_ptr, uint16_t c, uint16_t PtrTypes);
static void EncodePosition(intptr_t outfile_ptr, uint16_t c, uint16_t PtrTypes);
static void EncodeEnd(intptr_t outfile_ptr,uint16_t PtrTypes);


static int16_t GetByte(intptr_t infile_ptr, uint32_t *CompressLength, uint16_t PtrTypes);
static int16_t GetBit(intptr_t infile_ptr, uint32_t *CompressLength, uint16_t PtrTypes);	/* get one bit */
static int16_t DecodeChar(intptr_t infile_ptr, uint32_t *CompressLength, uint16_t PtrTypes);
static int16_t DecodePosition(intptr_t infile_ptr,uint32_t *CompressLength, uint16_t PtrTypes);
...
//
//   void VectorRoutine(uint32_t OriginalSize,uint32_t CurPosition)
//
//
...
int16_t far *prnt;
uint16_t far *freq;	/* cumulative freq table */
uint8_t far *text_buf;
...
static int16_t far *lson, far *rson, far *dad;
...
static void EncodePosition(intptr_t outfile_ptr, uint16_t c, uint16_t PtrTypes)
{
	...
	i = c >> 6;
	Putcode(outfile_ptr, p_len[i], (uint16_t)p_code[i] << 8,PtrTypes);
...
// LZH_Decompress()
//---------------------------------------------------------------------------
int32_t LZH_Decompress(void far *infile, void far *outfile, uint32_t OriginalLength, uint32_t CompressLength, uint16_t PtrTypes)
{
	int16_t  i, j, k, r, c;
	int32_t count;
	...
	{
		c = DecodeChar((intptr_t)&infile,&CompressLength,PtrTypes);

		if (c < 256)
		{
			CIO_WritePtr((intptr_t)&outfile,c,PtrTypes);
		...
		{
			i = (r - DecodePosition((intptr_t)&infile,&CompressLength,PtrTypes) - 1) & (N - 1);
			j = c - 255 + THRESHOLD;
...
int32_t LZH_Compress(void far *infile, void far *outfile,uint32_t DataLength,uint16_t PtrTypes)
{
	...
	{
		c = CIO_ReadPtr((intptr_t)&infile,PtrTypes);
	...
	do {
		...
		{
			match_length = 1;
			EncodeChar((intptr_t)&outfile,text_buf[r],PtrTypes);
		}
		else
		{
			EncodeChar((intptr_t)&outfile, 255 - THRESHOLD + match_length,PtrTypes);
			EncodePosition((intptr_t)&outfile, match_position,PtrTypes);
		}
	...
	EncodeEnd((intptr_t)&outfile,PtrTypes);
