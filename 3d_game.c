...
uint16_t	latchpics[NUMLATCHPICS];
...
//
// int16_t ElevatorBackTo[]={1,1,7,3,5,3};
...
/*** BLAKE STONE: ALIENS OF GOLD RESTORATION ***/
   ...
   boolean switchon = false;
	...
	/*** BLAKE STONE: ALIENS OF GOLD RESTORATION ***/
#ifdef GAMEVER_RESTORATION_AOG
	uint16_t	far *map,tile,spot;
#else
	uint16_t	far *map,tile,spot,icon;
#endif
	keytype lock;
	/*** BLAKE STONE: ALIENS OF GOLD RESTORATION ***/
#ifdef GAMEVER_RESTORATION_AOG
...
#ifdef GAMEVER_RESTORATION_AOG
				if (tile != AMBUSHTILE)
					actorat[x][y] = (objtype *)(size_t)(tile);
#else
...
//
	...
	/*** BLAKE STONE: ALIENS OF GOLD V1.0 RESTORATION ***/
#ifdef GAMEVER_RESTORATION_AOG_100
	LastInfoAttacker = SomeUnknownVar = alerted = NumEAWalls = 0;
#else
...
void ShadowPrintLocationText(sp_type type)
{
	...
	/*** BLAKE STONE: ALIENS OF GOLD V1.0 RESTORATION ***/
#ifdef GAMEVER_RESTORATION_AOG_100
	char str[8],far *s,*ls_text[2]={"-- LOADING --","-- SAVING --"};
#else
...
void StartDemoRecord (int16_t levelnumber)
{
...
/*** BLAKE STONE: ALIENS OF GOLD V3.00 RESTORATION ***/
	...
	int16_t dems[4]={T_DEMO1,T_DEMO2,T_DEMO3};
#else
...
/*** BLAKE STONE: ALIENS OF GOLD V3.00 RESTORATION ***/
#ifdef GAMEVER_RESTORATION_AOG_300
	int16_t dems[6]={T_DEMO1,T_DEMO2,T_DEMO3,T_DEMO4,T_DEMO5};
#else
...
/*#ifdef __AMIGA__
	gamestate.mapon = SWAP16LE(gamestate.mapon);
#endif*/
	gamestate.difficulty = gd_easy;
	//length = *((uint16_t far *)demoptr)++;
	length = *((uint16_t far *)demoptr);
	demoptr += 2;
#ifdef __AMIGA__
	length = SWAP16LE(length);
#endif
	demoptr++;
...
void Died (void)
{
	float	fangle;
	int32_t	dx,dy;
	int16_t		iangle,curangle,clockwise,counter,change;
...
//         StopMusic();			// JTR
		...
		case ex_victorious:
			/*** BLAKE STONE: ALIENS OF GOLD V1.0 RESTORATION ***/
			// More-or-less done after checking score in v1.0,
			// with a bit different string
#ifndef GAMEVER_RESTORATION_AOG_100
			MainMenu[MM_SAVE_MISSION].active=AT_DISABLED;
//#pragma warn -sus
			 MainMenu[MM_VIEW_SCORES].routine=(void (*)(int16_t))&CP_ViewScores;
...
//				VL_SetPalette (0,256,grsegs[ENDINGPALETTE]);
			...
			/*** BLAKE STONE: ALIENS OF GOLD V1.0 RESTORATION ***/
			// More-or-less done earlier in later versions,
			// with a bit different string
#ifdef GAMEVER_RESTORATION_AOG_100
//#pragma warn -sus
			_fstrcpy(MainMenu[MM_VIEW_SCORES].string,"VIEW SCORES");
