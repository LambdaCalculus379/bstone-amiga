...
#if (defined GAMEVER_RESTORATION_AOG_100) || IN_DEVELOPMENT
//#if IN_DEVELOPMENT
/*
================
=
...
// Looks like only PicturePause is present in v1.00S, more-or-less
...
/*** BLAKE STONE: ALIENS OF GOLD RESTORATION ***/
#if (defined GAMEVER_RESTORATION_AOG_POST100) || IN_DEVELOPMENT
//#if IN_DEVELOPMENT
/*
================
=
...
#ifdef __AMIGA__
		sprintf(buf,"0x%08x",(intptr_t)addr);
#else
...
//---------------------------------------------------------------------------
uint16_t IncRange(uint16_t Value,uint16_t MaxValue)
{
	if (Value == MaxValue)
   	Value = 0;
   else
...
// NOTE: Assumes that 0 is the lowest value
//---------------------------------------------------------------------------
uint16_t DecRange(uint16_t Value,uint16_t MaxValue)
{
	if (Value == 0)
		Value = MaxValue;
	else
...
// Note that this is in a different location for V1.00R (BS6),
// and with a few differences (in strings)
#ifndef GAMEVER_RESTORATION_AOG_100
...
/*** BLAKE STONE: ALIENS OF GOLD V1.00 RESTORATION ***/
// Note that this is in a different location for v1.0,
// and with a few differences (in strings)
#ifdef GAMEVER_RESTORATION_AOG_100
	else if (Keyboard[sc_D])			// D = Dumb/Blind Objects (Player Invisable)
	{
		...
		PlayerInvisable ^= 1;
		if (PlayerInvisable)
			US_PrintCentered ("Player Invisable!");
		else
			US_PrintCentered ("Player visable");
