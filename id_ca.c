...
uint16_t	_seg	*mapsegs[MAPPLANES];
...
extern	int32_t	far	CGAhead;
extern	int32_t	far	EGAhead;
extern	byte	CGAdict;
extern	byte	EGAdict;
...
void	CAL_CarmackExpand (uint16_t far *source, uint16_t far *dest,
		uint16_t length);


#ifdef THREEBYTEGRSTARTS
//#define	GRFILEPOS(c) (*(int32_t far *)(((byte far *)grstarts)+(c)*3)&0xffffff)
...
#if defined __AMIGA__ && BYTE_ORDER == BIG_ENDIAN
//#error restore me
	//value = 0;
	//memcpy(((byte *)&value) + 1, ((byte far *)grstarts)+offset, 3);
	value = *(int32_t far *)(((byte far *)grstarts)+offset);
	value &= 0xFFFFFF00;
...
void CAL_GetGrChunkLength (int16_t chunk)
{
	...
	chunkexplen = SWAP32LE(chunkexplen);
...
void CAL_CarmackExpand (uint16_t far *source, uint16_t far *dest, uint16_t length)
{
	uint16_t	ch,chhigh,count,offset;
	uint16_t	far *copyptr, far *inptr, far *outptr;
	...
	while (length)
	{
		...
		if (chhigh == NEARTAG)
		{
			...
			{				// have to insert a word containing the tag byte
				ch |= *((uint8_t far *)inptr)++;
				*outptr++ = ch;
				length--;
			}
			else
			{
				offset = *((uint8_t far *)inptr)++;
...
//
	...
	if (mmerror)
   {
	...
	expanded = SWAP32LE(*(int32_t far *)source);
	...
	if (chunk < STARTMUSIC) // Sound effects
	{
		...
		sndCommonPtr->length = SWAP32LE(sndCommonPtr->length);
		sndCommonPtr->priority = SWAP16LE(sndCommonPtr->priority);
#if 0
		if ((chunk >= STARTDIGISOUNDS) /*&& (chunk < STARTMUSIC)*/) // Digitized sounds
		{
			SampledSound *sampledSndPtr = (SampledSound *)audiosegs[chunk];
			sampledSndPtr->hertz = SWAP16LE(sampledSndPtr->hertz);
		}
#endif
		/*
		if (chunk >= STARTADLIBSOUNDS && chunk < STARTDIGISOUNDS)
		{
			AdLibSound *sound = sndCommonPtr;
		}
		*/
	}
	else // Music chunk
	{
		...
		musicPtr->length = SWAP16LE(musicPtr->length);
		// Swap the delays only
		for (word bytesLeft = musicPtr->length, *musicData = 1 + musicPtr->values; bytesLeft >= 4; bytesLeft -= 4, musicData += 2)
			*musicData = SWAP16LE(*musicData);
	}
#endif
...
#ifndef GAMEVER_RESTORATION_AOG_POST100
		goto cachein;
#endif
	case sdm_PC:
		start = STARTPCSOUNDS;
...
void CAL_ExpandGrChunk (int16_t chunk, byte far *source)
{
	int32_t	expanded;
...
#define MASKBLOCK	128
	...
	//
#ifdef __AMIGA__
		expanded = SWAP32LE(*(int32_t*)source);
#else
...
//
	...
	if (((chunk >= STARTFONT) && (chunk < STARTFONT+NUMFONT)) || ((chunk >= STARTFONTM) && (chunk < STARTFONTM+NUMFONTM)))
	{
		...
		font->height = SWAP16LE(font->height);
		for (int i = 0; i < (int)(sizeof(font->location)/sizeof(*(font->location))); ++i)
		{
			font->location[i] = SWAP16LE(font->location[i]);
		}
	}
...
//
	...
	CAL_HuffExpand (source,&vga_memory[vl_get_offset(bufferofs,0,0)],expanded,grhuffman,true);
...
//
	...
	for (plane = 0; plane<MAPPLANES; plane++)
	{
		...
		expanded = *source;
#ifdef __AMIGA__
		expanded = SWAP32LE(expanded);
#endif
		source++;
		MM_GetPtr (&buffer2seg,expanded);
		CAL_CarmackExpand (source, (uint16_t far *)buffer2seg,expanded);
#ifdef __AMIGA__
		uint16_t far * rlewunsignedptr = source;
		for (int i = 0; i < compressed/2; ++i, ++rlewunsignedptr)
		{
			*rlewunsignedptr = SWAP16LE(*rlewunsignedptr);
		}
#endif
		CA_RLEWexpand (((uint16_t far *)buffer2seg)+1,*dest,size,
