...
//uint16_t	plux,pluy;			// player coordinates scaled to uint16_t
...
void LoadOverheadChunk(int16_t tpNum);
void SaveOverheadChunk(int16_t tpNum);
void DisplayTeleportName(int8_t tpNum, boolean locked);
#endif
...
void DrawLedStrip(int16_t x,int16_t y,int16_t frac,int16_t max);
#endif
...
/*** BLAKE STONE: ALIENS OF GOLD RESTORATION ***/
#ifdef GAMEVER_RESTORATION_AOG
uint16_t far PulsePics[6] = {ECG_HEARTBEAT_00,ECG_HEARTBEAT_00,ECG_HEARTBEAT_00,ECG_HEARTBEAT_00,ECG_HEARTBEAT_00,ECG_HEARTBEAT_00};
#endif

//int16_t	strafeangle[9] = {0,90,180,270,45,135,225,315,0};
...
/*** BLAKE STONE: ALIENS OF GOLD RESTORATION ***/
#ifdef GAMEVER_RESTORATION_AOG
int8_t  far DimAmmo[2][21] = {{GD0,GD0,GD0,GD0,GD0,GD0,GD0,YD0,YD0,YD0,YD0,YD0,YD0,YD0,RD0,RD0,RD0,RD0,RD0,RD0,RD0},
									 {GD1,GD1,GD1,GD1,GD1,GD1,GD1,YD1,YD1,YD1,YD1,YD1,YD1,YD1,RD1,RD1,RD1,RD1,RD1,RD1,RD1}};
#else
int8_t  far DimAmmo[2][22] = {{GD0,GD0,GD0,GD0,GD0,GD0,GD0,YD0,YD0,YD0,YD0,YD0,YD0,YD0,RD0,RD0,RD0,RD0,RD0,RD0,RD0,RD0},
...
/*** BLAKE STONE: ALIENS OF GOLD RESTORATION ***/
#ifdef GAMEVER_RESTORATION_AOG
int8_t  far LitAmmo[2][21] = {{GL0,GL0,GL0,GL0,GL0,GL0,GL0,YL0,YL0,YL0,YL0,YL0,YL0,YL0,RL0,RL0,RL0,RL0,RL0,RL0,RL0},
									 {GL1,GL1,GL1,GL1,GL1,GL1,GL1,YL1,YL1,YL1,YL1,YL1,YL1,YL1,RL1,RL1,RL1,RL1,RL1,RL1,RL1}};
#else
int8_t  far LitAmmo[2][22] = {{GL0,GL0,GL0,GL0,GL0,GL0,GL0,YL0,YL0,YL0,YL0,YL0,YL0,YL0,RL0,RL0,RL0,RL0,RL0,RL0,RL0,RL0},
...
/*** BLAKE STONE: ALIENS OF GOLD RESTORATION ***/
#ifdef GAMEVER_RESTORATION_AOG
int8_t DrawHealthMonitor_COUNT = 3;
int8_t DrawHealthPic_COUNT = 3;
int8_t DrawPulse_COUNT = 3;
#endif
...
void SW_HandleStatic(statobj_t *stat,uint16_t tilex, uint16_t tiley);

//===========================================================================

//----------

byte ShowRatio(int16_t bx, int16_t by, int16_t px, int16_t py, int32_t total, int32_t perc, ss_type type);
void Attack (void);
void Use (void);
...
boolean ClipMove (objtype *ob, int32_t xmove, int32_t ymove);
...
==================
*/
void StatusAllDrawPic(uint16_t x, uint16_t y, uint16_t picnum)
{
	uint16_t	temp;
...
===============
*/
void	LatchNumber (int16_t x, int16_t y, int16_t width, int32_t number)
{
...
//
//--------------------------------------------------------------------------
void GivePoints(int32_t points,boolean add_to_stats)
{
...
void DrawKeyPics(void)
{
	/*** BLAKE STONE: ALIENS OF GOLD RESTORATION ***/
#ifdef GAMEVER_RESTORATION_AOG
	int16_t KeyX[] = {257, 265, 273, 281, 289};
	int16_t loop;
	int16_t KeyColors[] = {
		0x11, 0xC9,
		0x31, 0xB9,
	...
	for (loop=0; loop<NUMKEYS; loop++)
		...
		if (gamestate.numkeys[loop])
			VWB_Bar(KeyX[loop],177,7,7,*(int16_t *)&((byte *)KeyColors)[2+loop*4]);
		else
			VWB_Bar(KeyX[loop],177,7,7,*(int16_t *)&((byte *)KeyColors)[loop*4]);
#else
		if (gamestate.numkeys[loop])
...
//--------------------------------------------------------------------------
boolean DisplayInfoMsg(char far *Msg,msg_priorities Priority,int16_t DisplayTime,int16_t MsgType)
{
...
//	int16_t length,i;
	...
	char tmp[1024];
	strncpy(tmp, gamestate.msg, sizeof(tmp));
	first_ch = tmp; 
#else
...
//	piShapeInfo far *shape_info;
	...
	*(first_ch+1)=toupper(*(first_ch+1));
	...
	//switch (*((uint16_t far *)first_ch)++)
	uint16_t code = *((uint16_t far *)first_ch);
#ifdef __AMIGA__
	code = SWAP16LE(code);
#endif
	first_ch += 2;
...
// AnimatePage()
//--------------------------------------------------------------------------
void AnimatePage(int16_t numanims)
{
	piAnimInfo far *anim=piAnimList;
	piShapeInfo far *shape;
...
uint16_t far static_points[]={	100,		// money bag
											500,     // loot
											250,     // gold1
											500,     // gold2
											750,     // gold3
...
#endif
	{
		uint16_t keynum = check->itemnumber - bo_red_key;
...
{
	uint16_t offset, far *map, far *ptr[2], loop;
	byte areanumber;
	...
	//
		...
		//
...
#endif
			{
				...
				/*** BLAKE STONE: ALIENS OF GOLD V1.0 RESTORATION ***/
#ifdef GAMEVER_RESTORATION_AOG_100
				if ((new_floor=InputFloor(true)) != -1 && new_floor != gamestate.mapon)
#else
			...
			/*** BLAKE STONE: ALIENS OF GOLD V1.0 RESTORATION ***/
			// Separate case with duplicated code, only difference
			// being the argument that InputFloor ignores...
			//
			// Note: dir_horiz is checked here *only* in v1.0.
#ifdef GAMEVER_RESTORATION_AOG_100
			case TRANSPORTERTILE:
				if (dir_horiz)
			{
				...
				if ((new_floor=InputFloor(false)) != -1 && new_floor != gamestate.mapon)
				{
					playstate=ex_completed;
					gamestate.lastmapon=gamestate.mapon;
					gamestate.mapon=new_floor-1;
			...
			case FOODTILE:
			case SODATILE:
				OperateConcession((int16_t)(size_t)actorat[checkx][checky]);
			break;
...
#endif
{
	...
	int16_t *currTeleX,*currTeleY;
	int16_t teleX[MAX_TELEPORTS]={256,280, 256,280, 256,280, 256,280, 256,280, 232};
	int16_t teleY[MAX_TELEPORTS]={92,92, 72,72, 52,52, 32,32, 12,12, 92};
	int16_t buttonX=0,buttonY,buttonPic=0,rt_code=-1;
	int8_t flClass=0,flNum;
	boolean prepareStats=true;
	byte flScan=0,lastFlScan=0,loop,msgNum;

#ifndef GAMEVER_RESTORATION_AOG_100
	uint16_t oldwidth = viewwidth/16;
...
//			PollControls();
	...
	{
		int8_t shps[]={TELEPORT1ONPIC,TELEPORT1OFFPIC};
...
{
	int handle;
	int32_t cksize=4096+sizeof(statsInfoType);
	char chunk[5]="OVxx";
...
//
	sprintf(&chunk[2],"%02x",tpNum);
#ifdef __AMIGA__
	// some file systems don't like to open two write handles at the same time
	long oldpos;
	oldpos = tell(handle);
	close(handle);
	if ((handle=open(tempPath,O_RDONLY|O_BINARY))==-1)
		MAIN_ERROR(SAVELEVEL_DISKERR);
	lseek(handle,oldpos,SEEK_SET);
#endif
	DeleteChunk(handle,chunk);
#ifdef __AMIGA__
	// re-open the file for writing after deleting the chunk
	oldpos = tell(handle);
	close(handle);
	if ((handle=open(tempPath,O_CREAT|O_RDWR|O_BINARY,S_IREAD|S_IWRITE))==-1)
		MAIN_ERROR(SAVELEVEL_DISKERR);
...
/*** BLAKE STONE: ALIENS OF GOLD RESTORATION ***/
#ifdef GAMEVER_RESTORATION_AOG
	int16_t floor,total=0,mission=0,p1,p2,p3,loop;
#else
...
// ShowRatio()
//--------------------------------------------------------------------------
byte ShowRatio(int16_t bx, int16_t by, int16_t nx, int16_t ny, int32_t total, int32_t perc, ss_type type)
{
	int8_t numbars;
	int8_t maxperc;
...
static uint16_t far tcursor_x = TERM_SCREEN_XOFS,
...
//---------------------------------------------------------------------------
void CacheTerminalPrint(int16_t MsgNum,boolean FastPrint)
{
	char far *Message;
...
//	TerminalPrint("\r\n^XX",true);
	for (y=0;y<64;y++)
	{
		...
		for (x=0;x<64;x++)
		{
			...
			if (((uint16_t)actor && (uint16_t)actor<108) ||			// 108 == LAST WALL TILE
...
//	plux = player->x >> UNSIGNEDSHIFT;			// scale to fit in uint16_t
