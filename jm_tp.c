...
//
	if (pi->infoline)
	{
		int8_t oldf=fontnumber,oldc=fontcolor;
...
#endif
		uint16_t subcode;
		uint16_t code = *(uint16_t *)first_ch;
#ifdef __AMIGA__
		code = SWAP16LE(code);
#endif
		first_ch += 2;
				...
				while (*s && (*s != TP_RETURN_CHAR))
				{
					switch (*s)
					{
						case TP_CONTROL_CHAR:
							...
							subcode = *(uint16_t *)s;
#ifdef __AMIGA__
							subcode = SWAP16LE(subcode);
#endif
							s += 2;
							switch (subcode)
							//switch (*((uint16_t far *)s)++)
							{
...
// TP_DrawShape()
//--------------------------------------------------------------------------
int16_t TP_DrawShape(int16_t x, int16_t y, int16_t shapenum, pisType shapetype)
{
	int16_t width;
	memptr addr;
...
// TP_AnimatePage()
//--------------------------------------------------------------------------
void TP_AnimatePage(int16_t numanims)
{
...
// TP_BoxAroundShape()
//--------------------------------------------------------------------------
int16_t TP_BoxAroundShape(int16_t x1, int16_t y1, uint16_t shapenum, pisType shapetype)
{
	int16_t x2,y2;
...
#endif
				code = *(uint16_t *)script;
#ifdef __AMIGA__
				code = SWAP16LE(code);
#endif
				script += 2;
				switch (code) 
				//switch (*((uint16_t far *)script)++)
				{
					case TP_CNVT_CODE('S','H'):
...
// TP_VALUE()
//--------------------------------------------------------------------------
uint16_t TP_VALUE(char far *ptr,int8_t num_nybbles)
{
	int8_t ch,nybble,shift;
	uint16_t value=0;
...
// TP_SlowPrint()
//--------------------------------------------------------------------------
boolean TP_SlowPrint(char far *str, int8_t delay)
{
	int8_t old_color = fontcolor;
	int16_t old_x,old_y;
	int32_t tc;
...
// TP_LoadScript()
//--------------------------------------------------------------------------
int32_t TP_LoadScript(char *filename,PresenterInfo *pi, uint16_t id_cache)
{
//#pragma warn -pia
	int32_t size;
	...
	if (id_cache)
	{
		...
		size = (int32_t)(p - (const char*)pi->scriptstart - 1);
	}
	else
	{
		pi->id_cache = -1;
...
// TP_FreeScript()
//-------------------------------------------------------------------------
void TP_FreeScript(PresenterInfo *pi,uint16_t id_cache)
{
	TP_PurgeAllGfx();
...
// TP_CacheIn()
//-------------------------------------------------------------------------
void TP_CacheIn(tpCacheType type, int16_t chunk)
{
	int16_t first_ch_offset=first_ch-pi->script[0];
...
// TP_LineCommented()
//-------------------------------------------------------------------------
int16_t TP_LineCommented(char far *s)
{
	char far *o=s;
...
int16_t MDS_COMPUTERS_SUCK_SHIT()
{
	char far *fptr;
	void _seg *sptr;
	int16_t offset;
