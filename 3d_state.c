...
boolean ElevatorFloor(int8_t x, int8_t y);
...
void SpawnNewObj (uint16_t tilex, uint16_t tiley, statetype *state)
{
	...
	new->tilex = tilex;
	new->tiley = tiley;
	new->x = ((int32_t)tilex<<TILESHIFT)+TILEGLOBAL/2;
...
{                                                   \
	objtype *actor = actorat[x][y];                 \
	temp=(uint16_t)(size_t)actorat[x][y];                   \
	if (temp)                                       \
	{                                               \
		if (temp<256)    										 \
			return false;                           \
...
uint16_t far actor_points[]={	1025,				// rent-a-cop
										1050,				// turret
...
/*** BLAKE STONE: ALIENS OF GOLD RESTORATION ***/
	...
	case crate3obj:
		...
		ui16_to_static_object(ob->temp3)->shapenum = -1;		// Release reserve static
...
/*** BLAKE STONE: ALIENS OF GOLD RESTORATION ***/
	...
	if (ob->hitpoints<=0)
	{
		switch (ob->obclass)
		{
			...
			case gurney_waitobj:
				if (!(ob->temp2 = actor_to_ui16(CheckAndReserve())))
				{
					ob->hitpoints += damage;
					return;
				}
...
/*** BLAKE STONE: ALIENS OF GOLD RESTORATION ***/
		...
		SLIDE_TEMP(ob) = actor_to_ui16(attacker);
		KillActor (ob);
		return;
	}
	else
