...
#endif
#ifdef __AMIGA__
#undef SKIP_CHECKSUMS
#undef SHOW_CHECKSUM
#define SKIP_CHECKSUMS					(1)
#define SHOW_CHECKSUM					(1)
#endif
...
//	__SEGS_AVAILABLE__ = (int32_t)(mminfo.EMSmem + mminfo.XMSmem + mminfo.mainmem) / 16;
...
asm	jc	error
	...
	size = EMSAvail * (int32_t)EMSPageSize;
	if (size - (EMSPageSize * 2) > (ChunksInFile * (int32_t)PMPageSize))
	{
		size = (ChunksInFile * (int32_t)PMPageSize) + EMSPageSize;
	...
	mminfo.EMSmem += EMSAvail * (int32_t)EMSPageSize;
...
extern	byte					sbDMA,sbDMAa1,sbDMAa2,sbDMAa3,
						...
						sba2Vals[],
						sba3Vals[];
extern	int16_t						sbLocation,sbInterrupt,sbIntVec,sbIntVectors[];
extern	word				ssControl,ssStatus,ssData;
extern	byte				ssOn,ssOff;

static boolean SDL_DetectSoundBlaster(int16_t port);
...
extern int16_t USL_HardError(word errval,int16_t ax,int16_t bp,int16_t si);
...
asm	push	ax				// Delay a int while before we do this again
asm	pop		ax
asm	push	ax
asm	pop		ax
...
//
  ...
  lightsource=(byte far *)(((int32_t)colormap + 255)&~0xff);
...
//
	...
	{
		grhuffman[i].bit0 = SWAP16LE(grhuffman[i].bit0);
		grhuffman[i].bit1 = SWAP16LE(grhuffman[i].bit1);
	}
#endif
	CAL_OptimizeNodes (grhuffman);
//
...
#ifdef __AMIGA__
#ifndef THREEBYTEGRSTARTS
	for (int i = 0; i < (NUMCHUNKS+1)*FILEPOSSIZE; ++i)
	{
		grstarts[i] = SWAP32LE(grstarts[i]);
	}
#endif
#endif
...
//
	...
	{
		pictable[i].width = SWAP16LE(pictable[i].width);
		pictable[i].height = SWAP16LE(pictable[i].height);
	}
#endif
...
//
	...
	close(handle);
#ifdef __AMIGA__
	mapfiletype *tinfasmapfile = (mapfiletype *)tinf;
	tinfasmapfile->RLEWtag = SWAP16LE(tinfasmapfile->RLEWtag);
	for (int i = 0; i < sizeof(tinfasmapfile->headeroffsets)/sizeof(*(tinfasmapfile->headeroffsets)); ++i)
	{
		tinfasmapfile->headeroffsets[i] = SWAP32LE(tinfasmapfile->headeroffsets[i]);
	}
#endif
#else
...
#ifdef __AMIGA__
		mapheaderseg[i]->planestart[0] = SWAP32LE(mapheaderseg[i]->planestart[0]);
		mapheaderseg[i]->planestart[1] = SWAP32LE(mapheaderseg[i]->planestart[1]);
		mapheaderseg[i]->planestart[2] = SWAP32LE(mapheaderseg[i]->planestart[2]);
		mapheaderseg[i]->planelength[0] = SWAP16LE(mapheaderseg[i]->planelength[0]);
		mapheaderseg[i]->planelength[1] = SWAP16LE(mapheaderseg[i]->planelength[1]);
		mapheaderseg[i]->planelength[2] = SWAP16LE(mapheaderseg[i]->planelength[2]);
		mapheaderseg[i]->width = SWAP16LE(mapheaderseg[i]->width);
		mapheaderseg[i]->height = SWAP16LE(mapheaderseg[i]->height);
#endif
	}
...
extern int16_t far  CheckIs386(void);
...
//mmsize = mminfo.mainmem;
	...
	if (mminfo.mainmem < MIN_MEM_NEEDED)
	{
	 ...
	 ShutdownId();
#ifdef __AMIGA__
		printf("Out of memory! (%d / %ld)\n", mminfo.mainmem, MIN_MEM_NEEDED);
#else
...
void ShowSystem()
{
	...
	printf("     MAIN: %ld",(long)mminfo.nearheap+mminfo.farheap);
